...
| `SHUTDOWN_GRACE_MS` | `15000` | Plazo **único y compartido** entre las dos fases acotadas del apagado: parar los trabajos de fondo (#313) y drenar las peticiones en vuelo. Lo que consuma la primera se descuenta de la segunda, así que la espera hasta el cierre forzado es **como mucho este valor más 1 s**: el drenaje tiene un suelo de 1 s (`MIN_DRAIN_MS`) para poder terminar la petición en vuelo aunque los trabajos se hayan comido el plazo entero. Antes cada fase corría su propio plazo y la espera podía llegar al doble, más que el kill timeout del orquestador; al dimensionar ese timeout, cuenta `SHUTDOWN_GRACE_MS + 1 s`. |
...
| `cefiro_shared_mailbox_copies_total{result}` | contador | Copias automáticas de buzones compartidos (#313) intentadas, por desenlace: `copied`; `failed` (rechazada o con error del proveedor; el detalle va al log como `shared mailbox copy: copy refused/failed`, con `userId` y `emailId`; se reintenta en los ciclos siguientes hasta 5 veces); `skipped` (el mensaje llegó en una página repetida y no se copia: o el miembro ya tenía la copia confirmada, o su fila está `failed` y es del paso de reintentos, que es el único que la vuelve a copiar —comprueba antes por `Message-ID` y cuenta el intento—; una tasa que sube indica ciclos que se cortan a medias); `unresolved` (la fila del libro se reservó pero nunca se confirmó: el proceso murió o falló la base de datos entre la copia y su registro); `owed` (copia que la cuenta le **debe** a un miembro al que ese ciclo no puede servir —desactivado, sin credencial, sesión que falla—: no se llamó al proveedor, la fila queda como rastro y el paso de reintentos se la entrega al volver); y `dropped` (mensaje que ese rastro **no** llegó a registrar porque el miembro ya acumula el tope de filas debidas, `DELIVERY_OWED_CAP` = 1000). Una copia `unresolved` **no se reintenta** —la entrega es como mucho una vez, porque un mensaje duplicado es el fallo que el miembro nota— así que cualquier valor distinto de cero merece revisión: la recuperación es el botón manual de copiar a la bandeja. `dropped` es el único desenlace que **sí** es una pérdida de entrega: señala a alguien ilocalizable desde hace mucho (credencial revocada en el proveedor, cuenta que su sesión ya no lista) y lo que hay que arreglar es la membresía; el correo sigue en el buzón compartido. `owed` sostenido dice que alguien lleva tiempo sin poder recibir, no que se haya perdido nada. Las seis series existen desde el primer scrape, en cero. |
