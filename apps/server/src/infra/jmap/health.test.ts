...
describe("checkJmap", () => {
  it("probes the JMAP session well-known path on the configured base URL", async () => {
    ...
    await checkJmap({ url: "https://mail.test", fetchFn: fetchFn as unknown as typeof fetch });
  ...
  it("does not hang: a never-answering Stalwart resolves to unhealthy via the deadline", async () => {
    ...
    expect(await checkJmap({ url: "https://mail.test", timeoutMs: 20, fetchFn })).toBe(false);
  });
